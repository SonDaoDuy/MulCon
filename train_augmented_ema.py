...
def main():
    ...
    if opt.warm:
        rate_warmup = 0.1
        if opt.pretrain_folder is None:
            # end to end warm up
            lr_parts = [opt.lr*opt.lrp, opt.lr, opt.lr]
            opt.warmup_from = [opt.lr*opt.lrp*rate_warmup, opt.lr*rate_warmup, opt.lr*rate_warmup]
            for i in range(opt.num_classes):
                lr_parts.append(opt.lr)
                opt.warmup_from.append(opt.lr*rate_warmup)
        else:
            # finetune warm up
            lr_parts = [opt.lr*opt.lrp, opt.lr*opt.lrp, opt.lr]
            opt.warmup_from = [opt.lr*opt.lrp*rate_warmup, opt.lr*opt.lrp*rate_warmup, opt.lr*rate_warmup]
            for i in range(opt.num_classes):
                # lr_parts.append(opt.lr*opt.lrp)
                # opt.warmup_from.append(opt.lr*opt.lrp*rate_warmup)
                lr_parts.append(opt.lr)
                opt.warmup_from.append(opt.lr*rate_warmup)
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        # training
        all_pred = np.empty((0,opt.num_classes))
        all_label = np.empty((0,opt.num_classes))
        for i, (images, labels) in enumerate(dataset):
            ...
            data = {}
            data['img'] = torch.cat([images[0], images[1]], dim=0)
            data['gt_label'] = torch.cat([labels[0], labels[1]], dim=0)
            ...
            if total_iters % opt.print_freq == 0:
                ...
                # scores = metrics.average_precision(all_pred, all_label)
                #losses
