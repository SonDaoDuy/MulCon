...
def evaluate_all_ret(X_pred, X_true):

    ranks = [3,5,10,15,20]
    # ndcgs = np.zeros((len(ranks), X_pred.shape[0]))
    # precisions = np.zeros((len(ranks), X_pred.shape[0]))
    # recalls = np.zeros((len(ranks), X_pred.shape[0]))
    for i in range(len(ranks)):
        # ndcgs[i, :] = NDCG_binary_at_k_batch(X_pred, X_true, ranks[i])
        precisions = precisionAtK(X_pred, X_true, ranks[i])
        recalls = Recall_at_k_batch(X_pred, X_true, ranks[i])
        print('result at %d: ' % (ranks[i]))
        print(recalls.mean(), precisions)
        # print(len(recalls))
